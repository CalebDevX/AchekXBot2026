...
async function initChatbotData() {
  try {
    ...
    // NEW: Load explicitly disabled chats so global settings don't override them
    const chatbotOffData = config.CHATBOT_OFF || "";
    if (chatbotOffData) {
      const disabledChats = chatbotOffData.split(",").filter((jid) => jid.trim());
      disabledChats.forEach((jid) => {
...
async function saveChatbotData() {
  try {
    ...
    await setVar("CHATBOT_OFF", disabledChats.join(",")); // Save off states
  } catch (error) {
...
async function getAIResponse(message, chatJid, imageBuffer = null, audioBuffer = null) {
  ...
  try {
    ...
    const parts = [{ text: message || "Analyze this media" }];

    if (imageBuffer) {
      const imagePart = await imageToGenerativePart(imageBuffer, "image/jpeg");
    ...
    if (
      ...
      if (imageBuffer) contextMessage = `${message} [Image included]`;
      if (audioBuffer) contextMessage = `${message} [Audio included]`;
...
async function disableChatbot(jid) {
  chatbotStates.set(jid, false); // Explicit false
...
// NEW: Audio listener for Voice Notes
Module(
  {
    on: "audio",
    fromMe: false,
  ...
  async (message) => {
    try {
      ...
      const aiResponse = await getAIResponse("Listen to this audio and reply.", message.jid, null, audioBuffer);
      
      if (aiResponse && !aiResponse.includes("Rate limit reached")) {
        await message.sendReply(aiResponse);
      }
    } catch (error) {
      console.error("Audio handling error:", error.message);
    }
  }
);
...
Module(
  ...
  async (message, match) => {
    ...
    try {
      ...
      // This is using YOUR original ./utils/misc function, which works perfectly!
